            ...
            return {
              limit: async (_count: number) => {
                if (!columns.includes(state.ownerColumn)) {
                  return {
                    error: { code: '42703', message: `column \"${columns}\" does not exist` },
                  };
                }
...
describe('notifications edge function', () => {
  ...
  it('updates an existing token and preserves device_id when omitted', async () => {
    ...
    expect(state.tokens[0]).toEqual(
      expect.objectContaining({ device_id: 'device-a', enabled: true })
    );
  });
  ...
  it('supports multiple devices per user and disables stale token on the same device', async () => {
    const { admin, state } = createAdminClient({
      tokens: [
        {
          profile_id: 'user-1',
          token: 'ExpoPushToken[old-device-a]',
          device_id: 'device-a',
          enabled: true,
        },
        {
          profile_id: 'user-1',
          token: 'ExpoPushToken[device-b]',
          device_id: 'device-b',
          enabled: true,
        },
      ],
    ...
    expect(state.tokens.find(token => token.token === 'ExpoPushToken[old-device-a]')?.enabled).toBe(
      false
    );
    expect(state.tokens.find(token => token.token === 'ExpoPushToken[device-b]')?.enabled).toBe(
      true
    );
  ...
  it('rejects unauthenticated requests', async () => {
    ...
    const response = await handler(
      new Request('https://example.supabase.co/functions/v1/notifications/register-token', {
        method: 'POST',
  ...
  it('deletes a token for the authenticated user', async () => {
    const { admin, state } = createAdminClient({
      tokens: [
        { profile_id: 'user-1', token: 'ExpoPushToken[abc]', device_id: 'device-a', enabled: true },
      ],
    });
