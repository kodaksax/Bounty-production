...
describe('waitForAuthEvent', () => {
  ...
  it('handles Promise-returning onAuthStateChange registrations', async () => {
    const unsubscribeMock = jest.fn();
    let authCb: ((event: string, session: any) => void) | null = null;

    const onAuthStateChangeMock = jest.fn((cb: any) => {
      authCb = cb;
      return Promise.resolve({ data: { subscription: { unsubscribe: unsubscribeMock } } });
    });
    ...
    const p = waitForAuthEvent(5000);
    authCb?.('SIGNED_IN', { user: { id: 'async-registration' } });
    await Promise.resolve();
    const res = await p;

    expect(res).not.toBeNull();
    expect(res!.user.id).toBe('async-registration');
    expect(unsubscribeMock).toHaveBeenCalled();
  });
});
