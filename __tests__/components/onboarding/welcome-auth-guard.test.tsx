/**
 * Backstop for the "sent back to Welcome after signing up" failure.
 *
...
jest.mock('lib/context/onboarding-context', () => ({
  useOnboarding: () => ({
    data: { intent: null, experimentVariant: 'control' },
...
jest.mock('lib/posthog', () => ({ useFeatureFlag: () => 'control' }));
jest.mock('lib/experiments/first-screen-variant', () => ({
  useFirstScreenVariant: () => ({ variant: 'control', ready: true }),
}));
jest.mock('lib/haptic-feedback', () => ({
  hapticFeedback: { light: jest.fn() },
  useHapticFeedback: () => ({ triggerHaptic: jest.fn() }),
}));
jest.mock('components/ui/branding-logo', () => ({ BrandingLogo: 'BrandingLogo' }));
jest.mock('components/onboarding/PosterFirstWelcome', () => ({
  PosterFirstWelcome: 'PosterFirstWelcome',
}));
...
describe('onboarding welcome auth guard', () => {
  ...
  it('shows the normal welcome screen to a logged-out visitor', () => {
    const utils = render(<OnboardingWelcome />);
