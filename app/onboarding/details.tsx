...
export default function DetailsScreen() {
  ...
  // fetch error keeps the hunter on the location screen (with a retry banner).
  ...
  // Lets someone who picked the wrong path on welcome.tsx flip to the other
  // one without losing progress. Profile-level fields (name, bio, location,
  // skills, avatar) are shared across both paths and stay put; only fields
  // specific to the path being left behind are cleared, so switching back
  // and forth later doesn't resurrect a stale draft (e.g. a poster's task
  // description leaking into a hunter session). Also used by the hunter
  // dead-market fallback (nothing nearby to hunt yet).
  const handleSwitchIntent = (newIntent: 'poster' | 'hunter') => {
    analyticsService.trackEvent('onboarding_intent_switched', {
      from: onboardingData.intent ?? 'none',
      to: newIntent,
    });
    if (newIntent === 'poster') {
      setPosterStep('task');
      updateOnboardingData({ intent: newIntent });
    } else {
      setHunterStep('location');
      updateOnboardingData({ intent: newIntent, taskDescription: '', price: '', schedule: null });
    }
  };
