...
export function WalletScreen({ onBack }: WalletScreenProps = {}) {
  ...
  }, [session, hasValidSession, refreshFromApi, refreshBalance]);
  ...
  const onRefresh = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      // An explicit pull-to-refresh always hits Stripe, bypassing the
      // throttle that protects against focus-triggered refetch storms.
      await Promise.all([
        refreshFromApi(session!.access_token),
        refreshBalance({ force: true }),
      ]);
