...
import { AdminAttachmentGallery } from '../../../components/admin/AdminAttachmentGallery';
...
import type { ParsedBountyAttachments } from '../../../lib/admin/bounty-attachments';
...
export default function ModerationDetailScreen() {
  ...
  // The moderation RPC doesn't return attachments, and a moderator deciding
  // whether to hide or remove a listing has to be able to see its photos.
  // Best-effort: a failed read shows "No photos" rather than blocking review.
  const [attachments, setAttachments] = useState<ParsedBountyAttachments | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    adminDataClient
      .fetchBountyAttachments(bountyId)
      .then(setAttachments)
  ...
  return (
    <AdminScreen>
      ...
      <ScrollView contentContainerStyle={{ padding: theme.spacing.lg, paddingBottom: 64 }}>
        ...
        <AdminSection title="Photos & attachments">
          <AdminAttachmentGallery attachments={attachments} isLoading={attachmentsLoading} />
        </AdminSection>
