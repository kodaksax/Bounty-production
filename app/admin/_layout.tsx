...
//
// Admin lives under a real `/admin` URL segment, not an `(admin)` route group.
// Groups add no URL segment, so `(admin)/bounty/[id]` and `bounty/[id]` both
// answered `/bounty/:id` and expo-router resolved every bounty notification
// tap (`/bounty/:id?source=notification`) to the admin detail screen —
