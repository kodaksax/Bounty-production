...
/**
 * A `wallet_transactions` withdrawal row considered as the settlement target
 * of a Stripe Payout. Only the fields the match rules read.
 */
export interface TwoHopCandidateRow {
  id: string;
  /** Signed ledger amount; withdrawals are negative. Compared on absolute cents. */
...
export type TwoHopMatchResult =
  | { kind: 'match'; row: TwoHopCandidateRow }
  | { kind: 'none'; reason: 'no_destination' | 'no_candidates' | 'no_exact_match' }
  | { kind: 'ambiguous'; count: number };

/**
 * Chooses the single withdrawal a **standard two-hop** Payout settles, from
 * rows the caller has already narrowed to "pending, this user, no payout id
 * yet, transfer present, created before the payout".
 *
 * This exists because the payout id for a standard withdrawal does not exist
 * when the row is written: Stripe refuses API payout creation on a connected
 * account with an automatic payout schedule, so `/connect` stores
 * `metadata.payout_creation_failed` and leaves `stripe_payout_id` NULL, and
 * Stripe's scheduled payout settles hours or days later. Matching only by id
 * — the rule since 2026-08-16 — leaves those rows permanently unmatchable,
 * which is how withdrawal 205beb22 ($49.36) stayed `pending` for a week after
 * Stripe had paid it.
 *
 * It is emphatically NOT the amount-matching removed on 2026-08-16, which
 * took "most recent completed withdrawal with this amount" and attached a
 * payout to a row eleven days older. Two extra rules make the difference:
 *
 *  1. **Destination identity.** The row's stored `destination_bank_account_id`
 *     must equal the payout's destination. This is the discriminator the old
 *     heuristic never had.
 *  2. **Uniqueness.** Two surviving candidates yield `ambiguous`, never a
 *     guess. Callers must take no ledger action on `ambiguous` and leave it
 *     to reconciliation and a human.
 *
 * Exact-cents comparison is what keeps Stripe's sweep behaviour safe: an
 * automatic payout drains the connected account's whole balance, so a sweep
 * covering two withdrawals equals neither one and matches nothing.
 */
export function selectTwoHopWithdrawalMatch(args: {
  candidates: TwoHopCandidateRow[];
  payoutAmountCents: number;
  payoutDestination: string | null;
}): TwoHopMatchResult {
  // Without a destination there is no discriminator left, and amount alone is
  // exactly the unsound match this function exists to replace.
  if (!args.payoutDestination) return { kind: 'none', reason: 'no_destination' };
  if (args.candidates.length === 0) return { kind: 'none', reason: 'no_candidates' };

  const matches = args.candidates.filter((row) => {
    const cents = Math.round(Math.abs(Number(row.amount)) * 100);
    if (!Number.isFinite(cents) || cents !== args.payoutAmountCents) return false;
    const meta = row.metadata ?? {};
    return meta.destination_bank_account_id === args.payoutDestination;
  });

  if (matches.length === 1) return { kind: 'match', row: matches[0] };
  if (matches.length > 1) return { kind: 'ambiguous', count: matches.length };
  return { kind: 'none', reason: 'no_exact_match' };
}
