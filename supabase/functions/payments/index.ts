...
async function resolveStripeCustomerForUser(params: {
  ...
  if (profileError) {
    console.error('[payments] Failed to fetch profile during customer resolution', {
      userId,
      profileError,
    });
  }
  ...
  if (!profile && !profileError) {
    console.error('[payments] Profile row not found for user; cannot resolve Stripe customer', {
      userId,
    });
    return {
    ...
    try {
      ...
      if (err?.code === 'resource_missing') {
        ...
        try {
          ...
          console.error('[payments] Failed to clear stale stripe_customer_id', {
            userId,
            clearErr,
          });
        }
  ...
  if (saveError) {
    console.error('[payments] Failed to update profile while saving stripe_customer_id', {
      userId,
      saveError,
    });
    // Fallback: try a targeted update of just stripe_customer_id.
    try {
      const fallbackRes = (await withDbTimeout(
        supabaseAdmin.from('profiles').update({ stripe_customer_id: customerId }).eq('id', userId)
      )) as any;
      const fallbackError = fallbackRes?.error;
      if (fallbackError) {
        console.error('[payments] Fallback update of stripe_customer_id also failed', {
          userId,
          fallbackError,
        });
      } else {
        console.log('[payments] Fallback update of stripe_customer_id succeeded', {
          userId,
          customerId,
        });
      }
...
Deno.serve(async (req: Request) => {
  ...
  if (authError || !user) {
    ...
    console.warn(
      '[payments edge fn] invalid or expired token',
      JSON.stringify({
        requestId,
        hasUser: !!user,
  ...
  try {
    ...
    if (req.method === 'POST' && subPath === '/create-payment-intent') {
      ...
      try {
        ...
        return reply(
          { error: 'Invalid amount. Must be a positive number in cents.', code: 'invalid_amount' },
          400
        );
      }
      ...
      if (!['usd', 'eur', 'gbp'].includes(validatedCurrency)) {
        return reply(
          { error: 'Invalid currency. Supported: usd, eur, gbp.', code: 'invalid_currency' },
          400
        );
      }
      ...
      if (isAch) {
        ...
        if (!pmRow || pmRow.user_id !== userId) {
          return reply(
            { error: 'Bank account not found for this user.', code: 'payment_method_not_found' },
            403
          );
        }
        if (pmRow.type && pmRow.type !== 'us_bank_account') {
          return reply(
            {
              error: 'Selected payment method is not a bank account.',
    ...
    if (req.method === 'POST' && subPath === '/create-setup-intent') {
      ...
      try {
        ...
        console.error('[payments] Error creating Stripe customer for setup intent', {
          userId,
          err,
        });
    ...
    if (req.method === 'POST' && subPath === '/financial-connections-complete') {
      ...
      const sessionId = typeof body.sessionId === 'string' ? sanitizeText(body.sessionId) : '';
      ...
      for (const fcAccount of accounts) {
        ...
        const bankStatus = typeof usbankPm?.status === 'string' ? usbankPm.status : null;
        const bankVerificationStatus =
          typeof usbankPm?.verification_status === 'string' ? usbankPm.verification_status : null;
        const blocked = (usbankPm?.status_details as any)?.blocked === true;

        if (bankStatus === 'verified' || bankVerificationStatus === 'verified') {
    ...
    if (req.method === 'GET' && subPath === '/methods') {
      ...
      if (!profile?.stripe_customer_id) {
        const dbMethodsRes = (await withDbTimeout(
          supabaseAdmin
            .from('payment_methods')
            .select(
      ...
      try {
        ...
        if (err?.code === 'resource_missing') {
          console.warn(
            '[payments] Stripe customer missing for /payments/methods; clearing stripe_customer_id',
            {
              userId,
    ...
    if (req.method === 'POST' && subPath === '/confirm') {
      ...
      const recordDepositIfNeeded = async (intent: typeof paymentIntent) => {
        ...
        if (applyErr) {
          console.error('[payments/confirm] apply_deposit RPC failed', {
            userId,
            intentId: intent.id,
            applyErr,
          });
        ...
        if (appliedRow?.applied) {
          console.log('[payments/confirm] Deposit applied', {
            userId,
            intentId: intent.id,
            amountDollars,
            txId: appliedRow.tx_id,
          });
        } else {
          console.log('[payments/confirm] Deposit already recorded (idempotent no-op)', {
            userId,
            intentId: intent.id,
          });
        }
    ...
    if (err.type === 'StripeCardError') {
      return reply(
        { error: err.message, code: err.code ?? 'card_error', decline_code: err.decline_code },
        400
      );
    }
    if (err.code === 'resource_missing') {
    ...
    return reply(
      {
        ...
        code: err.code ?? 'payment_service_error',
        retryable: true,
      },
      500
    );
