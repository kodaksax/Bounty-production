...
// Use process.cwd() to resolve project root (avoids __dirname undefined in some ESLint configs)
loadEnv(process.cwd());
...
async function connect() {
  try {
    ...
    const dbPath = process.env.NODE_ENV === 'test'
      ? ':memory:'
      : path.join(projectRoot, 'data/bountyexpo.db');
...
async function initializeSchema(database) {
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS bounties (
        ...
        user_id TEXT, -- Legacy
        poster_id TEXT NOT NULL, -- New/Consistent
        username TEXT,
    ...
    ON bounties(poster_id, client_request_id)
    ...
    CREATE TABLE IF NOT EXISTS bounty_requests (
        ...
        hunter_id TEXT NOT NULL, -- Match Express server
        poster_id TEXT NOT NULL, -- Match Express server
        user_id TEXT, -- Legacy
        status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'accepted', 'rejected')),
        message TEXT, -- Match Express server
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (bounty_id) REFERENCES bounties(id) ON DELETE CASCADE,
        FOREIGN KEY (hunter_id) REFERENCES profiles(id) ON DELETE CASCADE,
    ...
    VALUES (
        '93955dc0-b933-4c6f-bf3c-4e01493169c0',
        '@poster007', 
        'poster@example.com',
        'Bounty Poster',
        '+998 90 943 32 00',
    ...
    INSERT OR IGNORE INTO profiles (id, username, email, about, phone, balance) 
    VALUES (
        'f4bd948b-a0a6-4991-8e5d-d4a3978760e6',
        '@grindhardlee', 
        'hunter@example.com',
        'Bounty Hunter',
        '+998 90 123 45 67',
  ...
  return {
    async execute(query, params = []) {
      ...
      let sqliteQuery = query
        ...
        .replace(/ON DUPLICATE KEY UPDATE.*/si, ''); // Strip MySQL-specific ON DUPLICATE KEY...
