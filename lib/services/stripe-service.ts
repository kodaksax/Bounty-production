...
async function invokePayments<T>(subPath: string, options: InvokePaymentsOptions = {}): Promise<T> {
  ...
  if (
    ...
    const invokeResult = await Promise.race<{ data: unknown; error: unknown } | null | undefined>([
    ...
    // Guard: if invoke() returned nothing (e.g. uninitialized stub in unit tests)
    // fall through to the last-resort fetch path rather than crashing on destructure.
    ...
    try {
      ...
      if (paymentIntent.status === 'requires_action') {
        ...
        const { paymentIntent: handledIntent, error } =
          await this.stripeSDK.handleNextAction(paymentIntentClientSecret);
  ...
  async isApplePaySupported(): Promise<boolean> {
    try {
      ...
      const isApplePaySupportedFn =
        this.stripeSDK.isApplePaySupported || this.stripeSDK.ApplePay?.isApplePaySupported;
  ...
  async presentApplePay(
    ...
    try {
      ...
      const presentApplePayFn =
        this.stripeSDK.presentApplePay || this.stripeSDK.ApplePay?.presentApplePay;
