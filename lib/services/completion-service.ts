...
async function postReadyViaRelayApi(bountyId: string, hunterId: string): Promise<boolean> {
  ...
  const response = await fetch(
    `${getRelayApiBaseUrl()}/bounties/${encodeURIComponent(bountyId)}/ready`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const completionService = {
  ...
  async submitCompletion(
    submission: Omit<CompletionSubmission, 'id' | 'submitted_at' | 'status'>
  ...
  subscribeSubmission(
    ...
    // initial fetch
    (async () => {
      const r = await completionService.getSubmission(bountyId);
      onUpdate(r);
    })();
    ...
    try {
      if (!bountyId || !hunterId) {
        logger.error('Error marking ready', {
          bountyId,
          hunterId,
          error: 'Missing bountyId or hunterId',
        });
        return false;
      }
        ...
        try {
          ...
          try {
            ...
            if (qb && typeof qb.upsert === 'function') {
              const { error: upsertErr } = await qb.upsert([payload], {
          ...
          const { error: insErr } = await supabase.from('completion_ready').insert(payload);
          ...
          if (/row-level security|permission|forbidden|not authorized/i.test(emsg)) {
            try {
              ...
              const message =
                edgeOrRelayErr instanceof Error ? edgeOrRelayErr.message : String(edgeOrRelayErr);
              throw new Error(message);
            }
          }
  ...
  subscribeReady(
    ...
    // Initial fetch
    (async () => {
      const r = await completionService.getReady(bountyId);
      onUpdate(r);
    })();
  ...
  async approveSubmission(
    bountyId: string,
    options?: { posterFeedback?: string; rating?: number }
  ): Promise<boolean> {
    try {
  ...
  async requestRevision(submissionId: string, feedback: string): Promise<boolean> {
    try {
      if (isSupabaseConfigured) {
        ...
        if (submission?.bounty_id) {
          try {
            ...
            let bountyConversation = all.find(
            ...
            if (!bountyConversation) {
              try {
                ...
                bountyConversation = await messageService.getOrCreateConversation(
                  [hunterId, posterId],
                  `Bounty ${submission.bounty_id}`,
                  submission.bounty_id
                );
              } catch (convErr) {
                logger.warning('Failed to auto-create bounty conversation for revision', {
                  error: convErr,
                });
              }
            }
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/completions/${submissionId}/request-revision`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async submitRating(rating: Omit<Rating, 'id' | 'created_at'>): Promise<Rating | null> {
    try {
      ...
      if (
        ...
        String(payload.from_user_id).trim() === '' ||
        payload.from_user_id === CURRENT_USER_ID
      ) {
        if (sessionUserId) {
      ...
      if (!payload.from_user_id || !payload.to_user_id) {
        throw new Error(
          'Missing required rating fields: from_user_id and to_user_id must be provided'
        );
      }
