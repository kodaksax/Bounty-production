  ...
  return (
    <View style={s.screen} className="flex-1">
      ...
      <ScrollView className="px-4" contentContainerStyle={{ paddingBottom: 96 }}>
        ...
        {/* ── Settings cards ───────────────────────────────────────────────── */}
        <SettingsCard theme={theme} title="Edit Profile"
          description="Modify your personal information, profile picture, contact details, and role preferences."
          primaryLabel="Save Changes" secondaryLabel="View My Profile"
          onPrimary={handleEditProfile} onSecondary={handleEditProfile} icon="person" />

        <SettingsCard theme={theme} title="Privacy & Security Settings"
          description="Manage your account's privacy and security, including password changes and two-factor authentication."
          primaryLabel="Open" onPrimary={() => setPanel('privacy')} icon="lock" />

        <SettingsCard theme={theme} title="Notifications Center"
          description="Aggregates all in-app notifications such as new applicants, task assignments, and payment updates."
          primaryLabel="Open" onPrimary={() => setPanel('notifications')} icon="notifications" />

        {/* ── Appearance ──────────────────────────────────────────────────── */}
        <View style={s.card} className="rounded-xl p-4 mb-4">
          <View className="flex-row items-center mb-3">
            <MaterialIcons name="palette" size={22} color={theme.primaryLight} />
            <Text style={s.cardTitle} className="ml-2 font-medium text-sm">Dark / Light mode</Text>
          </View>
          <View className="flex-row gap-2">
            {([
              ...
              ['dark',   '🌙', 'Dark'],
              ['system', '⚙️', 'System'],
            ] as [ThemeMode, string, string][]).map(([value, icon, label]) => {
        ...
        <SettingsCard theme={theme} title="Help & Support"
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center flex-1">
                ...
                <View className="ml-2 flex-1">
                  <Text style={s.cardTitle} className="font-medium text-sm">Admin Tab</Text>
                  <Text style={s.cardDescription} className="text-xs leading-4 mt-1" numberOfLines={2}>
                    {isAdminTabEnabled
        ...
        <SettingsCard theme={theme} title="Legal: Terms & Privacy"
          description="Read our Terms of Service and Privacy Policy."
          primaryLabel="View" onPrimary={() => setPanel('terms')} icon="gavel" />

        <SettingsCard theme={theme} title="Community Guidelines"
          description="Learn about our community standards for safety, trust, and respectful behavior."
          primaryLabel="View Guidelines"
          onPrimary={() => router.push('/legal/community-guidelines')} icon="security" />

        <SettingsCard theme={theme} title="Log Out"
        ...
        <SettingsCard theme={theme} title="Delete Account"
              ...
              [
                { text: 'Cancel', style: 'cancel' },
                {
                  ...
                  onPress: async () => {
                    ...
                    Alert.alert('Final Confirmation', 'This will permanently delete your account. Are you absolutely sure?', [
                      { text: 'Cancel', style: 'cancel' },
                      {
                        ...
                        onPress: async () => {
                          try {
                            ...
                            try { await authProfileService.clearUserDraftData(currentUserId) } catch {}
                            try {
...
function SettingsCard({ theme, title, description, primaryLabel, secondaryLabel, onPrimary, onSecondary, icon }: SettingsCardProps) {
  const s = makeStyles(theme)
  return (
    <View style={s.card} className="rounded-xl p-4 mb-4">
