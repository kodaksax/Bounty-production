  ...
  return (
    <View className="flex flex-col min-h-screen bg-emerald-600 text-white">
      ...
      <View className="flex-row items-center justify-between p-4 pt-8">
        ...
        <TouchableOpacity accessibilityRole="button" onPress={handleSave} className="p-2">
      ...
      <View className="flex-row items-center justify-between px-4 pb-2">
        ...
        <View className="flex-row">
          <TouchableOpacity accessibilityRole="button" onPress={addNewSkill} className="px-3 py-2 bg-emerald-700/60 rounded-lg mr-2">
            <Text className="text-white text-sm">Add</Text>
          </TouchableOpacity>
          <TouchableOpacity accessibilityRole="button" onPress={handleSave} className="px-3 py-2 bg-emerald-500 rounded-lg">
          ...
          return (
            <View key={skill.id} className={`mb-4 rounded-xl p-4 ${isActive ? 'bg-emerald-700/60 border border-emerald-300' : 'bg-emerald-700/30'}`}> 
              <View className="flex-row items-center mb-3">
                <TouchableOpacity accessibilityRole="button" onPress={() => setSelectedSkill(skill.id)} className="h-12 w-12 rounded-full bg-black/30 items-center justify-center mr-3">
                ...
                <TouchableOpacity accessibilityRole="button" onPress={() => removeSkill(skill.id)} className="ml-2 p-2">
              ...
              {isActive && (
                <View>
                  ...
                  <View className="flex-row flex-wrap -m-1 mb-3">
                    {ICON_LIBRARY.map(ic => (
                      <TouchableOpacity accessibilityRole="button" key={ic} onPress={() => changeIcon(skill.id, ic)} className={`m-1 h-9 w-9 rounded-lg items-center justify-center ${skill.icon === ic ? 'bg-emerald-500' : 'bg-emerald-800/60'}`}> 
                        <MaterialIcons name={ic as any} size={18} color={skill.icon === ic ? '#052e1b' : '#d1fae5'} />
                      </TouchableOpacity>
                    ))}
                  </View>
                  <View className="flex-row">
                    <TouchableOpacity accessibilityRole="button" onPress={() => attachCredential(skill.id)} className={`flex-1 ${skill.credentialUrl ? 'mr-2' : ''} px-3 py-2 bg-emerald-800/60 rounded-lg flex-row items-center justify-center`}>
                    ...
                    {skill.credentialUrl && (
                      <TouchableOpacity accessibilityRole="button" onPress={() => removeCredential(skill.id)} className="flex-1 px-3 py-2 bg-red-600/70 rounded-lg flex-row items-center justify-center">
