...
export function useGetBounties(options: GetBountiesOptions = {}) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const response = await apiClient.getBounties({ ...options, page: nextPage });
      ...
      if (response.success) {
        ...
        setState(prev => ({
          ...prev,
          loading: false,
          error: response.error || 'Failed to load more bounties',
        }));
      }
    } catch (error) {
      setState(prev => ({
  ...
  }, [apiClient, options, state.loading, state.pagination]);
...
export function useCreateBounty() {
  ...
  const createBounty = useCallback(async (bountyData: CreateBountyRequest): Promise<Bounty | null> => {
...
export function useAcceptBounty() {
  ...
  const acceptBounty = useCallback(async (request: AcceptBountyRequest): Promise<BountyRequest | null> => {
    setState({ data: null, loading: true, error: null });
...
export function useCompleteBounty() {
  ...
  const completeBounty = useCallback(async (request: CompleteBountyRequest): Promise<Bounty | null> => {
    setState({ data: null, loading: true, error: null });
