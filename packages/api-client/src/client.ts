...
export class ApiClient {
  ...
  async getBounties(options: GetBountiesOptions = {}): Promise<PaginatedResponse<Bounty>> {
    ...
    const response = await this.request<PaginatedResponse<Bounty>>(endpoint);
   ...
   * Update a bounty
   */
  async updateBounty(id: number, updates: Partial<CreateBountyRequest>): Promise<ApiResponse<Bounty>> {
    return this.request<Bounty>(`/api/bounties/${id}`, {
      method: 'PATCH',
   ...
   * Delete a bounty
   */
  async deleteBounty(id: number): Promise<ApiResponse<void>> {
    return this.request<void>(`/api/bounties/${id}`, {
      method: 'DELETE',
    });
  }

  /**
   * Get bounty requests for a specific bounty
   */
  async getBountyRequests(bountyId: number): Promise<ApiResponse<BountyRequest[]>> {
    return this.request<BountyRequest[]>(`/api/bounties/${bountyId}/requests`);
  }

  /**
   * Update bounty request status (accept/reject)
   */
  async updateBountyRequestStatus(
    requestId: number, 
    status: 'accepted' | 'rejected'
  ): Promise<ApiResponse<BountyRequest>> {
    return this.request<BountyRequest>(`/api/bounty-requests/${requestId}`, {
      method: 'PATCH',
