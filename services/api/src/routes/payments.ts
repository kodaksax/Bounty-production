...
export async function registerPaymentRoutes(fastify: FastifyInstance) {
  ...
  if (!stripeKey) {
    ...
    // Guard against reply being undefined so handlers can be invoked directly
    // in unit tests without a full Fastify reply object.
    const notConfigured = async (_req: any, reply: any) => {
      if (!reply) return { error: 'Stripe not configured on this server' };
      return reply.code(501).send({ error: 'Stripe not configured on this server' });
    };

    fastify.post('/payments/create-payment-intent', notConfigured);
    fastify.post('/payments/create-setup-intent', notConfigured);
    fastify.get('/payments/methods', notConfigured);
    fastify.delete('/payments/methods/:paymentMethodId', notConfigured);
    fastify.post('/payments/confirm', notConfigured);

    return;
  }

  // Detect key mode and log warning if there might be a mismatch
  const secretKeyMode = stripeKey.startsWith('sk_test_')
    ? 'test'
  ...
  const publishableKeyMode = backendPublishableKey.startsWith('pk_test_')
    ? 'test'
    : backendPublishableKey.startsWith('pk_live_')
      ? 'live'
      : 'unknown';
  ...
  if (
    ...
    logger.error(
      `[payments] KEY MODE MISMATCH: Secret key is in ${secretKeyMode} mode but backend publishable key (STRIPE_PUBLISHABLE_KEY) is in ${publishableKeyMode} mode.`
    );
    logger.error(
      '[payments] Please ensure STRIPE_SECRET_KEY and STRIPE_PUBLISHABLE_KEY are both test keys or both live keys.'
    );
    logger.error(
      '[payments] Also verify that EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY in the mobile app matches the secret key mode.'
    );
  } else if (
    secretKeyMode === 'unknown' ||
    (backendPublishableKey && publishableKeyMode === 'unknown')
  ) {
    logger.warn(
      '[payments] Unable to determine Stripe key mode from STRIPE_SECRET_KEY and/or STRIPE_PUBLISHABLE_KEY. Keys may have an unexpected format.'
    );
    logger.warn('[payments] STRIPE_SECRET_KEY should start with "sk_test_" or "sk_live_".');
    logger.warn(
      '[payments] STRIPE_PUBLISHABLE_KEY (and EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY in the mobile app) should start with "pk_test_" or "pk_live_".'
    );
  } else {
    logger.info(`[payments] Stripe configured in ${secretKeyMode} mode`);
    if (!backendPublishableKey) {
      logger.info(
        '[payments] Backend STRIPE_PUBLISHABLE_KEY not set (optional). Make sure EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY in mobile app is in the same mode.'
      );
    }
  }
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      ...
      try {
        ...
        logger.info(
          `[payments] Created PaymentIntent ${paymentIntent.id} for user ${request.userId}, amount: ${amountCents}`
        );
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        if (error && (error.type || error.code || error.message)) {
          const status =
            error.type === 'StripeInvalidRequestError' || error.type === 'validation_error'
              ? 400
              : 500;
  ...
  fastify.delete(
    ...
    async (request: AuthenticatedRequest, reply) => {
      try {
        ...
        logger.info(
          `[payments] Detached payment method ${paymentMethodId} for user ${request.userId}`
        );
        ...
        if (purpose === 'wallet_deposit') {
          try {
            ...
            if (looksLikeDuplicate) {
              ...
              try {
                const existing =
                  await ConsolidatedWalletService.getTransactionByPaymentIntent(paymentIntentId);
                if (existing) {
                  logger.info(
                    { existingId: existing.id },
                    '[payments] Deposit already recorded by webhook'
                  );
                } else {
                  logger.error(
                    { err: txError },
                    '[payments] Failed to record deposit (not duplicate)'
                  );
                ...
                logger.error(
                  { err: checkErr },
                  '[payments] Error checking existing deposit after createDeposit failure'
                );
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      ...
      try {
        ...
        logger.info(
          `[payments] Created escrow PaymentIntent ${paymentIntent.id} for bounty ${bountyId}`
        );
  ...
  fastify.post(
    ...
    async (request: AuthenticatedRequest, reply) => {
      ...
      try {
        ...
        logger.info(
          `[payments] Released escrow ${escrowId}, transferred ${hunterAmountCents} cents to hunter ${hunterId}`
        );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!rawBody) {
          logger.warn(
            '[payments] rawBody not available - ensure Fastify rawBody plugin is configured'
          );
        }
        const body =
          rawBody ||
        ...
        switch (event.type) {
          case 'payment_intent.succeeded': {
            ...
            if (paymentIntent.metadata.purpose === 'wallet_deposit') {
              try {
                ...
                const msg = String(txError?.message || txError || '');
                const looksLikeDuplicate = /duplicate|unique|already exists|duplicate key/i.test(
                  msg
                );
                ...
                if (looksLikeDuplicate) {
                  logger.info(
                    { err: txError },
                    '[payments] Duplicate deposit detected in webhook, ignoring'
                  );
                } else {
                  // As a fallback, try to see if a transaction already exists
                  try {
                    const existing = await ConsolidatedWalletService.getTransactionByPaymentIntent(
                      paymentIntent.id
                    );
                    if (existing) {
                      logger.info(
                        { existingId: existing.id },
                        '[payments] Deposit already recorded'
                      );
                    } else {
                      ...
                      logger.error(
                        { err: txError },
                        '[payments] Failed to record deposit in webhook (logged, continuing)'
                      );
                    }
                  } catch (checkErr: any) {
                    logger.error(
                      { err: checkErr },
                      '[payments] Error checking existing deposit in webhook'
                    );
                    ...
                    logger.error(
                      { err: txError },
                      '[payments] Failed to record deposit in webhook after check error (logged, continuing)'
                    );
                  }
                }
              }
          ...
          case 'payment_intent.payment_failed': {
            ...
            if (isEscrow && bountyId) {
              ...
              if (updateError) {
                logger.error(
                  { err: updateError.message, bountyId, paymentIntentId: paymentIntent.id },
                  '[payments] Failed to revert bounty status after escrow payment failure'
                );
              } else {
                logger.info(
                  { bountyId, paymentIntentId: paymentIntent.id },
          ...
          case 'setup_intent.succeeded': {
            ...
            if (userId && setupCustomerId) {
              ...
              if (profileSyncError) {
                logger.error(
                  { err: profileSyncError.message, userId, customerId: setupCustomerId },
            ...
            if (userId && paymentMethodId) {
              try {
                ...
                const { error: upsertError } = await admin.from('payment_methods').upsert(
                  {
                    user_id: userId,
                ...
                if (upsertError) {
                  logger.error(
                    { err: upsertError.message, userId, paymentMethodId },
                ...
                logger.error(
                  { err: pmErr?.message, userId, paymentMethodId },
                  '[payments] Error retrieving/saving payment method after setup_intent.succeeded'
                );
              }
            } else {
              logger.warn(
                { setupIntentId: setupIntent.id, userId, paymentMethodId },
          ...
          case 'charge.refunded': {
            ...
            if (prefetchError) {
              logger.error(
                { err: prefetchError.message, chargeId: charge.id },
                '[payments] Failed to fetch existing refund records — aborting to prevent duplicate processing'
              );
              throw new Error(
                `Failed to fetch existing refunds for charge ${charge.id}: ${prefetchError.message}`
              );
            }

            const processedRefundIds = new Set(
              (processedRefunds || []).map((r: any) => r.metadata?.refund_id).filter(Boolean)
            );
            ...
            for (const refund of refunds) {
              ...
              if (insertError) {
                logger.error(
                  { err: insertError.message, refundId: refund.id, chargeId: charge.id },
              ...
              try {
                await ConsolidatedWalletService.updateBalance(
                  originalTx.user_id,
                  -refundAmountDollars
                );
              } catch (balanceErr: any) {
                logger.error(
                  { err: balanceErr?.message, refundId: refund.id, userId: originalTx.user_id },
          ...
          case 'account.updated': {
            ...
            if (lookupError) {
              logger.error(
                { error: lookupError.message, accountId: account.id },
                '[payments] Failed to look up profile for Connect account'
              );
              break;
            }
            ...
            if (!profile) {
              logger.warn(
                { accountId: account.id },
                '[payments] No profile found for Connect account — skipping sync'
              );
              break;
            }
            ...
            const onboardingUpdate =
              onboardingComplete && !profile.stripe_connect_onboarded_at
            ...
            if (updateError) {
              logger.error(
                { error: updateError.message, accountId: account.id, userId: profile.id },
                '[payments] Failed to sync Connect account status to profile'
              );
              break;
            }

            logger.info(
              { accountId: account.id, userId: profile.id, onboardingComplete },
              '[payments] Connect account status synced to profile'
            );

            // Notify hunter only if the currently_due requirements have changed since last sync
            const prevCurrentlyDue: string[] =
              (profile.stripe_connect_requirements as Stripe.Account.Requirements | null)
                ?.currently_due ?? [];
            const requirementsChanged =
              currentlyDue.length > 0 &&
              JSON.stringify([...currentlyDue].sort()) !==
                JSON.stringify([...prevCurrentlyDue].sort());
            ...
            if (requirementsChanged) {
              try {
                ...
                logger.warn(
                  ...
                  '[payments] Failed to send Connect action-required notification'
                );
              }
            }
            break;
          ...
          case 'review.closed': {
            const review = event.data.object as Stripe.Review;
            logger.info(
              `[payments] Review closed for payment intent ${review.payment_intent}, reason: ${review.closed_reason}`
            );
            break;
          }
