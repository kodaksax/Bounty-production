...
7. **`profiles` SELECT RLS is effectively unrestricted** for any authenticated (or anon) caller across all 62 columns, including `balance`, `stripe_connect_account_id`, `risk_level`, `email`, `phone` — a read-side counterpart to a write-side hole that was fixed 2026-07-17. **Partially fixed 2026-07-18**: the underlying RLS policy / column grants are unchanged (still `USING (true)`, still no column REVOKE), but every identified cross-user consumer that depended on broad `profiles.select('*')` access has been migrated off it — `public_profiles` formalized as a tracked migration, a new service-role-backed `admin-profiles` Edge Function for the admin panel, and safe-column selects in `auth-profile-service.ts`/`user-search-service.ts`/`bounty-request-service.ts`. This means the actual column-level `REVOKE` (the step that closes the hole) is now much lower-risk than before, but is **still not applied** — see `docs/withdrawals/08-profiles-rls-migration-strategy.md`'s 2026-07-18 update for why (a self-read gap: users loading their own balance/email via the anon-key client would also break under a blanket REVOKE, and that path needs its own fix first).
...
- **`profiles` SELECT RLS — open finding, cataloged and partially remediated**: the base table's SELECT policies are still `USING (true)` across all 62 columns, including `balance`, `stripe_connect_account_id`, `stripe_customer_id`, `risk_level`, `email`, `phone`, and this is still readable by any authenticated (or anon) caller today — the RLS/grant change itself has not been applied. What changed 2026-07-18: every legitimate cross-user profile read identified in the app was migrated onto a safe-columns view or a service-role Edge Function instead of relying on this broad policy, which means the policy tightening is no longer blocked on "will this break something" — it's blocked on a genuine self-read gap (see `docs/withdrawals/08-profiles-rls-migration-strategy.md`) that needs its own fix first.
