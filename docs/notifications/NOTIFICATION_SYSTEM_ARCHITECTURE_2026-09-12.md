...
| Bounty stale (2h / 12h, zero applications) | `fn_escalate_stale_bounty_liquidity` (pg_cron, every 20 min) | Broadens the notified pool (2x radius -> Anywhere hunters for paid jobs -> flat 50mi); stops permanently once the bounty has any application. For `work_type='online'` bounties the candidate pool aggregates every non-deleted profile unfiltered and uncapped -- the 90-day-activity/500-row bound from `20260912152257_bound_online_liquidity_escalation_candidates.sql` was deliberately reverted in prod on 2026-09-19 and is not present in the live definition (`20260919120000_liquidity_escalation_requires_candidates.sql`). `liquidity_stage` only advances when `fn_score_and_dispatch_bounty_notification` reports it dispatched to at least one hunter (BNTY-02, same migration); a pass that reached nobody stamps `liquidity_last_escalated_at` and retries on the normal cadence. An `in_person` bounty with `geom IS NULL` has no candidate pool at any stage, so it is parked at its current stage, the poster gets one `bounty_location_nudge` (`bounties.liquidity_location_nudged_at`), and a `bounty_liquidity_no_candidates` warning is upserted into `reconciliation_findings` for admins |
