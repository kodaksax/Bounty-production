...
| [12-comprehensive-audit-2026-07-18.md](12-comprehensive-audit-2026-07-18.md) | Everyone (new engineer / support / ops) | Single self-contained answer to a full 17-question / 13-deliverable audit — executive summary, user journey, customer FAQ, security assessment, financial reconciliation, launch readiness, outstanding risks, automation recommendations, success criteria. Independently re-verified against live production this pass (deployed source byte-diffed against git, live RLS/migrations/cron-job state, live table contents) rather than restating 01-11. **Two new findings surfaced here, not elsewhere in this set:** the daily reconciliation cron job has zero recorded executions despite being `active`; 0 of 88 live profiles currently have `payouts_enabled = true`. |
| [13-instant-cash-out.md](13-instant-cash-out.md) | Engineers / product | Instant Cash Out (debit-card payout, typically minutes not days) — design, what's new vs. the standard flow above, failure semantics, and the rollout checklist before `INSTANT_CASHOUT_ENABLED` goes live. Not live as of this doc. Also documents an unrelated governance finding from the same session: two edge functions (`stripe-setup`, `stripe-worker`) live in production with no git history and unrecoverable source. |
| [14-instant-payouts-continuation-2026-07-21.md](14-instant-payouts-continuation-2026-07-21.md) | Engineers / product | Continuation session: audits and live-verifies the Instant Payout work landed just before it, fixes one real bug (misleading "reusable for payouts" bank-linking copy), closes a `connect` deploy-drift gap, runs the live Stripe test-mode permission suite, and gives a full Stripe Express compatibility matrix answering "what can/can't be unified between payment and payout methods." Flags the one remaining unverified path: no debit card has ever actually been linked to the test fixture account, so a real instant-payout Stripe call has never been exercised end-to-end. |
